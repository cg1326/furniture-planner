...
export function Toolbar() {
  ...
  return (
    <>
      <div className="h-11 bg-zinc-900 flex items-center gap-1 px-3 flex-shrink-0 relative">
        ...
        <div className="flex items-center gap-2 pr-3 mr-1 border-r border-zinc-700/60">
          ...
          <span className="text-sm font-semibold text-white tracking-tight">Floor Planner</span>
        </div>

        {/* Undo / Redo */}
        <button onClick={undo} disabled={past.length === 0} title="Undo (⌘Z)"
          className="p-1.5 rounded text-zinc-400 hover:text-white hover:bg-zinc-700/60 disabled:opacity-25 disabled:cursor-not-allowed transition-colors">
        ...
        <button onClick={toggleScaleUnit} title="Toggle units"
          className="px-2.5 py-1 rounded text-xs font-medium text-zinc-300 hover:text-white hover:bg-zinc-700/60 transition-colors">
          {scaleUnit}
        </button>
        <button onClick={() => setStageTransform(1, 40, 40)}
          className="px-2.5 py-1 rounded text-xs font-medium text-zinc-400 hover:text-white hover:bg-zinc-700/60 transition-colors">
          Reset view
        </button>
        ...
        <button
          ...
          className="p-1.5 rounded text-zinc-400 hover:text-white hover:bg-zinc-700/60 transition-colors"
        >
          <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
        ...
        {saving ? (
          <div className="flex items-center gap-1.5 ml-1">
            <input
              ...
              className="h-7 w-40 bg-zinc-800 border border-zinc-600 rounded-md px-2 text-xs text-white focus:outline-none focus:border-blue-500"
          ...
          <span
            className="ml-1 text-xs text-zinc-400 truncate max-w-[140px] cursor-default"
        ...
        <div className="ml-auto flex items-center gap-2">
          <span className="text-xs text-zinc-500 tabular-nums">{Math.round(stageScale * 100)}%</span>
          <div className="w-px h-4 bg-zinc-700/60" />
          <button onClick={clearAll}
            className="px-2.5 py-1 rounded text-xs font-medium text-zinc-400 hover:text-red-400 hover:bg-red-500/10 transition-colors">
            Clear all
          </button>
        </div>
      </div>
